...
export async function googleBooksByIsbn(isbn: string, userId: string): Promise<{ book?: Book; error?: string; }> {
  ...
  const book = {
    book: {
      ...
      description: volumeInfo.description || volumeInfo.subtitle || "No description available.",
      imageUrl: `https://covers.openlibrary.org/b/isbn/${isbn}-M.jpg`, //volumeInfo.imageLinks?.thumbnail,
      genre: volumeInfo.categories || null,
      imageHint: json.items[0].searchInfo?.textSnippet || null,
      userId: userId,
      weight: "",
      height: volumeInfo.dimensions?.height || null,
      width: volumeInfo.dimensions?.width || null,
      length: volumeInfo.dimensions?.length || null,
      tag: "",
      price: json.items[0].saleInfo?.retailPrice?.amount || null,
      asin: "",
      isbn10: ""
    },
    error: ""
